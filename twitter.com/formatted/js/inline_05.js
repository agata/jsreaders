WATCH('loadrunner', function() {
    (function(context, document) {
        var useInteractive = context.attachEvent && !context.opera,
            ...
            scriptsInProgress = {},
            modulesInProgress = {},
            loadedModule, currentScript, activeScripts = {},
            oldUsing = context.using,
            oldProvide = context.provide,
            oldDefine = context.define,
        ...
        Script.prototype.start = function() {
            ...
            if (dep = scriptsInProgress[this.id]) {
                ...
                if (!this.force && indexOf(Script.loaded, this.id) > -1) {
                    this.loaded()
                } else {
                    this.load()
                }
        ...
        Script.prototype.complete = function() {
            ...
            if (this.times) {
                Script.times[this.id] = aug(this.times, {
                    end: new Date()
                })
            }
        ...
        Collection.prototype.as = function(cb) {
            ...
            return this.then(function() {
                var flatDeps = flattenDeps(me.deps),
                    obj = {};
        ...
        function amdDefine() {
            ...
            var mod = defineModule(id, function(exports) {
                ...
                function executeAMD() {
                    var args = amdMap(makeArray(dependencies), mod),
                        exported;
        ...
        function whichBundle(id) {
            for (var manifestId = 0; manifestId < using.bundles.length; manifestId++) {
                for (var bundleId in using.bundles[manifestId]) {
                    if (bundleId != id && indexOf(using.bundles[manifestId][bundleId], id) > -1) {
                        return bundleId
                    }
                }
            }
    ...
    loadrunner(function(D, C, B, E) {
        function A(F) {
            this.id = this.path = F
        }
        A.inProgress = [];
        A.done = [];
        A.prototype = new B.Dependency;
        A.prototype.start = function() {
            var F = this,
                G;
            if (G = A.done[this.id]) {
                this.complete(G.result)
            } else {
                if (G = A.inProgress[this.id]) {
                    G.then(function() {
                        F.complete(G.result)
                    })
                } else {
                    A.inProgress[this.id] = this;
                    this.load()
                }
            }
        };
        A.prototype.load = function() {
            ...
            H.onreadystatechange = function() {
                if (H.readyState == 4) {
                    F.result = H.responseText;
                    A.done[F.id] = F;
                    delete A.inProgress[F.id];
                    F.complete(F.result)
                }
            };
            H.open("GET", this.path, true);
            H.send(null)
        };
        D.matchers.add(/^json!/, function(F) {
            return new A(F.substring(5))
        })
    });
    loadrunner(function(D, C, B, E) {
        function A(F) {
        ...
        A.prototype.load = function() {
            var F = this;
            D("lib/lazyload", function(G) {
                if ($("link[href='" + F.path + "']").length > 0) {
                    H()
                } else {
                    G.css(F.path, H)
                }
        ...
        D.matchers.add(/^css!/, function(F) {
            return new A(F.replace(/^css!/, "").replace(/^\//, (D.cssPath || "").replace(/\/$/, "") + "/"))
        })
    });
    loadrunner(function(F, E, A, G) {
        function B(H, I) {
            for (var J = 0, K; K = H[J]; J++) {
                if (I == K) {
                    return J
                }
            }
        ...
        C.inProgress = [];
        C.done = [];
        C.prototype = new A.Dependency;
        C.prototype.start = function() {
            var I, H = this;
            if (B(C.done, this.id) != -1) {
                this.complete()
            } else {
                if (I = C.inProgress[this.id]) {
                    I.then(function() {
                        H.complete()
                    })
                } else {
                    C.inProgress[this.id] = this
                }
            }
        };
        F.matchers.add(/^>/, function(H) {
            return new C(H.substring(1))
        });
        window.signal = D
    });
});
